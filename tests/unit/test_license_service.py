...
from assets.runtime.hardened.license_service import LicenseError, LicenseVerifier, generate_keypair, issue_license
...
def setup_license(tmp_path, **overrides):
    ...
    verifier = LicenseVerifier(public_key_path=public, license_path=license_path, state_path=state, clock=lambda: NOW)
...
def test_expired_license(tmp_path):
    verifier, *_ = setup_license(tmp_path, expires_at_utc=NOW - timedelta(seconds=1))
...
def test_old_cache_does_not_override_renewed_license(tmp_path):
    ...
    verifier.verify()
    renewed = issue_license(private_key_path=private, client_id="CLIENTE-PRUEBA", license_id="LIC-RENOVADA", issued_at_utc=NOW, expires_at_utc=NOW + timedelta(days=365))
    write_json_atomic(path, renewed, backup=False)
    new_verifier = LicenseVerifier(public_key_path=public, license_path=path, state_path=state, clock=lambda: NOW)
...
def test_legacy_license_requires_migration(tmp_path):
    ...
    write_json_atomic(path, {"licencia": "legacy", "expira": "2027-01-01"}, backup=False)
