...
class ScanQueue:
    def __init__(self, *, writer, capacity: int = 64, logger: Callable[[str], None] | None = None, notify: Callable[[str, str], None] | None = None):
    ...
    def cancel_for_configuration_change(self, new_generation: int) -> int:
        self.cancel_current(f"configuracion_cambio_a_generacion_{new_generation}")
        return self.clear_pending(f"configuracion_cambio_a_generacion_{new_generation}")

    def status(self) -> QueueStatus:
        with self._lock:
            current = self._current.sequence_id if self._current else None
            return QueueStatus(self._queue.qsize(), not self._pause.is_set(), current, self._completed, self._failed, self._cancelled)
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                if result.success:
                    ...
                    self._completed += 1
                    self.logger(f"queue_completed:{job.sequence_id}:elapsed_ms={result.elapsed_ms:.1f}")
                elif self._current_cancel.is_set():
                    job.state = JobState.CANCELLED
                    job.cancellation_reason = job.cancellation_reason or result.reason or "cancelado"
                    self._cancelled += 1
                    ...
                    self._failed += 1
                    self.notify("No se escribió el documento", "El formulario objetivo cambió o no pudo validarse")
                    self.logger(f"queue_failed:{job.sequence_id}:{job.failure_reason}")
